...
@test "_core_account_sync_render_summary groups by type with status badges" {
    run_in_zsh '
        printf "mcp\tgithub\tgithub\tnew\n" >/tmp/cs.$$
        printf "mcp\tvibma\tvibma\toverwrite\n" >>/tmp/cs.$$
        printf "claude-md\tCLAUDE.md\tCLAUDE.md\tnew\n" >>/tmp/cs.$$
        cat /tmp/cs.$$ \
          | _core_account_sync_render_summary src dst /tmp/backup-dir-stub /tmp/no-summaries
        rm -f /tmp/cs.$$'
    [[ "$output" == *"MCP servers"* ]]
    [[ "$output" == *"[+]"* ]]
    [[ "$output" == *"github"* ]]
...
@test "_core_account_sync_render_summary suppresses unchanged rows by default" {
    run_in_zsh '
        printf "mcp\tgithub\tgithub\tnew\n" >/tmp/cs.$$
...
@test "_core_account_sync_count_changes returns total + new + overwrite" {
    run_in_zsh '
        printf "mcp\tgithub\tgithub\tnew\n" >/tmp/cs.$$
        printf "mcp\tvibma\tvibma\toverwrite\n" >>/tmp/cs.$$
        printf "settings\tmodel\tmodel\tnew\n" >>/tmp/cs.$$
        cat /tmp/cs.$$ | _core_account_sync_count_changes
        rm -f /tmp/cs.$$'
    [[ "$output" == *"3 2 1"* ]]
}

@test "_core_account_sync_drill_down_items emits only [~] (overwrite) rows" {
    run_in_zsh '
        printf "mcp\tgithub\tgithub\tnew\n" >/tmp/cs.$$
        printf "mcp\tvibma\tvibma\toverwrite\n" >>/tmp/cs.$$
        printf "settings\tmodel\tmodel\toverwrite\n" >>/tmp/cs.$$
        cat /tmp/cs.$$ | _core_account_sync_drill_down_items
        rm -f /tmp/cs.$$'
    [[ "$output" != *"github"* ]]
    [[ "$output" == *"vibma"* ]]
    [[ "$output" == *"model"* ]]
