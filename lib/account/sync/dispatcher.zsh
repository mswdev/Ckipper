...
typeset -g _CKIPPER_SYNC_FROM=""
typeset -ga _CKIPPER_SYNC_TARGETS=()
typeset -g _CKIPPER_SYNC_INCLUDE=""
typeset -g _CKIPPER_SYNC_EXCLUDE=""
typeset -g _CKIPPER_SYNC_DRY_RUN="false"
...
_ckipper_account_sync_reset_args() {
    _CKIPPER_SYNC_FROM=""; _CKIPPER_SYNC_TARGETS=()
    _CKIPPER_SYNC_INCLUDE=""; _CKIPPER_SYNC_EXCLUDE=""
    _CKIPPER_SYNC_DRY_RUN="false"; _CKIPPER_SYNC_YES="false"; _CKIPPER_SYNC_FORCE="false"
    _CKIPPER_SYNC_CTX=()
}

# Append a positional arg to _CKIPPER_SYNC_FROM (first time) or _CKIPPER_SYNC_TARGETS (every
...
_ckipper_account_sync_accumulate_positional() {
    [[ -z "$_CKIPPER_SYNC_FROM" ]] && { _CKIPPER_SYNC_FROM="$1"; return 0; }
    _CKIPPER_SYNC_TARGETS+=("$1")
}
...
_ckipper_account_sync_parse_args() {
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            --include) _CKIPPER_SYNC_INCLUDE="$2"; shift 2 ;;
            --exclude) _CKIPPER_SYNC_EXCLUDE="$2"; shift 2 ;;
            --dry-run) _CKIPPER_SYNC_DRY_RUN="true"; shift ;;
            --yes)     _CKIPPER_SYNC_YES="true";     shift ;;
            --force)   _CKIPPER_SYNC_FORCE="true";   shift ;;
...
_ckipper_account_sync_run() {
    if [[ -z "$_CKIPPER_SYNC_FROM" ]]; then
        _CKIPPER_SYNC_FROM=$(_ckipper_account_sync_pick_source) || return 1
    fi
...
_ckipper_account_sync_run_targets() {
    ...
    local rc=0 target
    for target in "${_CKIPPER_SYNC_TARGETS[@]}"; do
        _ckipper_account_sync_validate_pair "$_CKIPPER_SYNC_FROM" "$target" || { rc=1; continue; }
...
_ckipper_account_sync_validate_accounts() {
    _core_account_dir "$_CKIPPER_SYNC_FROM" >/dev/null || return 1
    local t
    for t in "${_CKIPPER_SYNC_TARGETS[@]}"; do
...
_ckipper_account_sync_resolve_types() {
    if [[ -z "$_CKIPPER_SYNC_INCLUDE" && "$_CKIPPER_SYNC_YES" != "true" && "$_CKIPPER_SYNC_DRY_RUN" != "true" ]]; then
        _ckipper_account_sync_pick_types
        return 0
    fi
    [[ -z "$_CKIPPER_SYNC_INCLUDE" ]] && _CKIPPER_SYNC_INCLUDE="all"
    _ckipper_account_sync_resolve_includes "$_CKIPPER_SYNC_INCLUDE" "$_CKIPPER_SYNC_EXCLUDE"
}
...
_ckipper_account_sync_run_one_target() {
    ...
    src_dir=$(_core_account_dir "$_CKIPPER_SYNC_FROM")
    ...
    if [[ "$_CKIPPER_SYNC_DRY_RUN" != "true" ]]; then
        _ckipper_account_sync_assert_dst_idle "$dst_dir" "$_CKIPPER_SYNC_FORCE" || return 1
    fi
    _ckipper_account_sync_prepare_target_artifacts "$target" "$src_dir" "$dst_dir" "$@"
    local action="apply"
    if [[ "$_CKIPPER_SYNC_DRY_RUN" != "true" && "$_CKIPPER_SYNC_YES" != "true" ]]; then
        action=$(_ckipper_account_sync_preview_prompt)
    fi
    [[ "$_CKIPPER_SYNC_DRY_RUN" == "true" ]] && action="dry-run"
    _ckipper_account_sync_finalize "$action"
}

# Initialize per-target context and build the diff/summary artifacts.
# Steps: mktemp the changeset/summaries/items tmpfiles, populate _CKIPPER_SYNC_CTX,
...
_ckipper_account_sync_show_preview() {
    ...
    local backup_path
    backup_path=$(_ckipper_account_sync_backup_dir_path "$dst_dir" "$_CKIPPER_SYNC_FROM")
    _ckipper_account_sync_render_summary "$_CKIPPER_SYNC_FROM" "$target" \
...
#
# Reads _CKIPPER_SYNC_CTX[dst_name] for the prompt label. Callers capture this
...
_ckipper_account_sync_preview_prompt() {
    local target="${_CKIPPER_SYNC_CTX[dst_name]}"
    local choice=""
    while true; do
...
# tmpfile paths and apply args from _CKIPPER_SYNC_CTX.
#
# Args: $1 — action.
...
_ckipper_account_sync_finalize() {
    ...
    if [[ "$action" == "apply" ]]; then
        _ckipper_account_sync_apply_target \
            "${_CKIPPER_SYNC_CTX[src_dir]}" "${_CKIPPER_SYNC_CTX[dst_dir]}" \
            "${_CKIPPER_SYNC_CTX[src_name]}" "${_CKIPPER_SYNC_CTX[dst_name]}" \
            < "${_CKIPPER_SYNC_CTX[changeset]}"
        rc=$?
    fi
    rm -f "${_CKIPPER_SYNC_CTX[changeset]}" "${_CKIPPER_SYNC_CTX[summaries]}" "${_CKIPPER_SYNC_CTX[items]}"
    return $rc
}
...
#
# Force is read from a LOCAL var, not the module-level _CKIPPER_SYNC_FORCE. The
# previous design read _CKIPPER_SYNC_FORCE directly, which leaked across
# invocations: a prior `sync ... --force` left _CKIPPER_SYNC_FORCE="true" in the
...
# _CKIPPER_SYNC_FORCE on the sync path, but the undo path skips parse_args.
#
