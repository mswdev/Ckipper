#!/usr/bin/env bats
# Module-level tests for lib/w/ports.zsh.
# Covers _w_bind_port success path and fallback when ports are busy.
...
@test "_w_bind_port appends a -p flag to W_DOCKER_ARGS when port is free" {
    _run_ports '_w_bind_port 3000; print -r -- "${W_DOCKER_ARGS[*]}"'

    [ "$status" -eq 0 ]
    [[ "$output" =~ "-p 127.0.0.1:3000:3000" ]]
}

@test "_w_bind_port logs a warning when all fallback ports are busy" {
